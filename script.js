document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- UPDATED SCENARIO DATABASE - No neutral choices, better balanced ---
    const SCENARIOS = [
    ...
    const startGameBtn = document.getElementById('startGameBtn'), restartGameBtn = document.getElementById('restartGameBtn'), playAgainBtn = document.getElementById('playAgainBtn'), exitTownBtn = document.getElementById('exitTownBtn');
    ...
    // --- UPDATED NARRATIVE EVENT LOGIC with smart scenario selection ---
    function triggerNarrativeEvent(pos) {
        eventSpots = eventSpots.filter(s => s.q !== pos.q || s.r !== pos.r);
        currentView = 'narrative';
        
        // Smart scenario selection - filter scenarios based on what player can afford
        const availableScenarios = SCENARIOS.filter(scenario => {
            return scenario.choices.some(choice => isChoiceAvailable(choice));
        });
        
        // If no scenarios are affordable, pick from positive scenarios that don't require resources
        const fallbackScenarios = SCENARIOS.filter(scenario => {
            return scenario.choices.some(choice => !choice.condition);
        });
        
        const scenarioPool = availableScenarios.length > 0 ? availableScenarios : fallbackScenarios;
        ...
        choiceA_btn.style.display = 'block';
        
        // Choice B
        choiceB_btn.textContent = choices[1].text;
        choiceB_btn.disabled = !isChoiceAvailable(choices[1]);
        choiceB_btn.style.display = 'block';
        
        // Choice C - always hidden now since we removed neutral choices
    ...
    // Town, UI, Game State Functions - RUMOR SYSTEM REMOVED
    ...
    function updateTownInterface() {
        ...
        ITEM_KEYS.forEach(itemKey => {
            const item = ITEMS[itemKey]; 
            const price = currentSettlement.prices[itemKey]; 
            const playerAmount = player.inventory.get(itemKey) || 0; 
            const isDesperate = itemKey === currentSettlement.desperateFor; 
        ...
        marketDiv.querySelectorAll('.buy-btn').forEach(b => b.addEventListener('click', e => buyItem(e.target.dataset.item, parseInt(e.target.dataset.price)))); 
        marketDiv.querySelectorAll('.sell-btn').forEach(b => b.addEventListener('click', e => sellItem(e.target.dataset.item, parseInt(e.target.dataset.price))));
...
// --- EVENT LISTENERS (RUMOR SYSTEM REMOVED) ---
...
exitTownBtn.addEventListener('click', exitTown);
choiceA_btn.addEventListener('click', () => resolveChoice(0)); 
choiceB_btn.addEventListener('click', () => resolveChoice(1)); 
choiceC_btn.addEventListener('click', () => resolveChoice(2));
